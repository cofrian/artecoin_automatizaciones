...
class FotoExtractorExcel:
    ...
    max_workers: int = 10  # Número de hilos concurrentes
    ...
    def get_sheets_data(self) -> Dict[str, pd.DataFrame]:
        ...
        # Pre-cache de columnas de fotos para evitar recalcular
        self._photo_columns_cache = {}
        for sheet_name, df in dataframes.items():
            self._photo_columns_cache[sheet_name] = self._extract_photo_columns(df)
    ...
    def _download_photo_from_url(
        ...
        try:
            ...
            filename = self._extract_filename_from_url(url, sheet_name, row_index, column_name, df)
    ...
    def _download_photos_for_center_in_sheet(
        ...
        if hasattr(self, '_photo_columns_cache'):
            photo_columns = self._photo_columns_cache.get(sheet_name, [])
        else:
            photo_columns = self._extract_photo_columns(df)
        ...
        # Crear lista de todas las descargas pendientes
        download_tasks = []
        for col in photo_columns:
            ...
            for idx in matching_rows.index:
                ...
                if url and str(url).strip():
                    download_tasks.append((url, center_id, sheet_name, idx, col, df, matching_rows, col))
        ...
        if download_tasks:
            with ThreadPoolExecutor(max_workers=self.max_workers) as executor:
                ...
                for future in as_completed(future_to_task):
                    ...
                    col = task[7]  # El nombre de la columna
                    try:
                        result = future.result()
                        results[col].append(result)
                    except Exception as exc:
                        error_msg = f"   ✗ Error en descarga: {exc}"
                        results[col].append(error_msg)
    ...
    def download_all_photos(
        ...
        """
        Descarga todas las fotos organizadas por centro ID con paralelización completa.
        ...
        # Crear todas las tareas de procesamiento
        processing_tasks = []
        for center_id in center_ids:
            for sheet_name, df in dataframes.items():
                processing_tasks.append((center_id, sheet_name, df))

        # Procesar múltiples centros/hojas simultáneamente
        with ThreadPoolExecutor(max_workers=min(len(processing_tasks), 20)) as executor:
            future_to_task = {
                executor.submit(
                    self._download_photos_for_center_in_sheet,
                    task[0], task[1], task[2]
                ): task for task in processing_tasks
            }

            for future in as_completed(future_to_task):
                center_id, sheet_name, _ = future_to_task[future]
                try:
                    results = future.result()
                    if results:
                        if center_id not in all_results:
                            all_results[center_id] = {}
                        all_results[center_id][sheet_name] = results
                except Exception as exc:
                    with self._print_lock:
                        print(f"Error procesando centro {center_id}, hoja {sheet_name}: {exc}")
...
if __name__ == "__main__":
    # Crear instancia del extractor con más workers para máximo rendimiento
    extractor = FotoExtractorExcel(EXCEL_PATH, max_workers=15)

    # Cargar datos de Excel
    dataframes = extractor.get_sheets_data()

    # Descargar fotos con paralelización completa
    all_photos = extractor.download_all_photos(dataframes)

    # Cerrar session al final
